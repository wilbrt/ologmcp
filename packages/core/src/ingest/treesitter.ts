...
function collectTypeIdentifiers(node: Parser.SyntaxNode): string[] {
  const result: string[] = [];
...
function walkDescendants(node: Parser.SyntaxNode, visitor: (n: Parser.SyntaxNode) => void): void {
  for (const child of node.children) {
    visitor(child);
...
function extractPropertyFromNode(
  node: Parser.SyntaxNode,
  parentName: string,
  parentKind: string,
 ...
 * in a source file. Returns one PropertyExtract per field.
 *
 * @param parser     - A Parser configured for the file's language
 * @param source     - The source code string to parse
 * @param _moduleName - Module path (unused; kept for API symmetry)
 */
export function extractPropertiesFromFile(
  ...
  walkDescendants(tree.rootNode, (node) => {
    if (node.type === 'interface_declaration') {
      ...
      // Interface body is an object_type
      ...
      if (!typeNode) return;
      // Only extract from object types (not union/intersection of literals)
