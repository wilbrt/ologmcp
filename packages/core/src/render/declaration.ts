...
import { parserFor } from '../ingest/treesitter.js';

const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);
const TS_QUERY_PATH = resolve(__dirname, '..', 'ingest', 'queries', 'ts.scm');
const TSX_QUERY_PATH = resolve(__dirname, '..', 'ingest', 'queries', 'tsx.scm');
...
export function extractDeclaration(
  ...
  const range = findEnclosingDeclaration(source, filePath, identifierLine, identifierCol, kind);
  return range?.text ?? null;
}